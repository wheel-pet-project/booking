...
public sealed class Booking : Aggregate
{
    ...
    public FreeWait FreeWait { get; } = null!;
    public DateTime? Start { get; private set; }
    public DateTime? End { get; private set; }

    public void MarkAsNotBooked()
    {
        if (Status.CanBeChangedToThisStatus(Status.NotBooked) == false)
    ...
    public void Book(TimeProvider timeProvider)
    {
        ...
        if (Status.CanBeChangedToThisStatus(Status.Booked) == false)
            throw new DomainRulesViolationException(
    ...
    public void Expire(TimeProvider timeProvider)
    {
        ...
        bool FreeWaitExpired()
        {
            return timeProvider.GetUtcNow().UtcDateTime > Start.Value.Add(FreeWait.Duration.ToTimeSpan());
        }
    }
    ...
    public static Booking Create(Customer customer, VehicleModel vehicleModel, Guid vehicleId)
    {
        ...
        if (customer.CanBookThisVehicleModel(vehicleModel) == false)
            throw new DomainRulesViolationException(
